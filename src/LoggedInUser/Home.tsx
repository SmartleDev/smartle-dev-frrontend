            ...
            <Box style={{borderBottom: '0.83px dashed #917EBD'}}>
                <Box >
                    {myCourses.length === 0 ? <div>
                        ...
                        <p className='loggedhome_subheading dark:text-white'>You are not enrolled in any courses. Begin your learning journey by exploring courses below.</p>
                        <Box style={{textAlign:'center', marginTop: '20px', marginBottom: '80px'}}>
                            <Link to={"/courses"} >
                                <button
                    ...
                    <div>
                      <h2 className="loggedhome_heading dark:text-white">
                        ...
                        <div className="my-courses" style={{ display: "flex", flexWrap: "wrap", marginLeft: '35px', marginTop:'20px'}}>
        {myCourses.length === 0 ?
          null
         : myCourses?.map((dataItem: any, index: number) => (
          <>
            <div style = {{position: "relative", height : '400px'}}>
              ...
              <div style={{ width: "380px" }} className="p-2 relative">
               ...
               {dataItem?.enrollment_type === 'paid' &&<div style = {{position: 'absolute', top : '0px', width: "370px"}}>
                <BorderLinearProgress variant="determinate" value={dataItem.course_progress} className='progress_bar'/>
            ...
            <Grid container spacing={2} sx={{borderBottom: '0.83px dashed #917EBD'}}>
              <Grid item xs={12} md={6}>
                <Box margin="20px" sx={{borderRight: '0.83px dashed #917EBD'}}>
                    <Typography fontWeight={700} fontSize="32px" className = 'section_heading dark:text-white'>Our Top courses</Typography>
                    {
          <div style={{display : 'flex', justifyContent:'center', flexWrap: 'wrap', borderRadius:'5px'}}>
          {courses?.map((course:any, index: number) => 
          <Link to={`/course/${course.course_title}`}   >                
                    <div className={`${width} md:p-3 mb-5 recommended_courses`} style={{borderRadius:'5px'}}>
                      ...
                      <h2  className='courseFontStyle dark:text-white'>{course.course_title}</h2>
                      <br />
                      <br />
              ...
              <Grid item xs={12} md={6}>
                <Box margin="20px" className='dark:text-white'>
                  <Typography fontWeight={700} fontSize="32px">Key events</Typography>
                  <Box style={{borderRadius: '20px', boxShadow: '4px 4px 10px rgba(0, 0, 0, 0.15)', padding: '20px', marginTop:'20px'}}>
                      ...
                      {keyEvents?.map((event) =>{
                        ...
                        return (
                          <Box style={{boxShadow: '4px 4px 10px #DFD1E7', borderRadius:'12px', padding:'10px', marginTop:'20px'}}>
                            <Typography maxWidth={"250px"} fontSize={"22px"} fontWeight={'900'} style={{color:"#917EBD", paddingLeft:'10px'}}>{courseName}</Typography>
                            <Grid container style={{marginTop:'30px'}}>
                              <Grid item xs={12} md={6} padding="10px">
                                  <Box style={{
                                      ...
                                      color: '#917EBD',
                                      cursor:'pointer'
                                      }}><Button href={event.session_zoomurl} target="_blank" style={{color:'#917EBD', fontSize:'16px'}}>Join Session</Button></Box>
                              </Grid>
                              <Grid item xs={12} md={6} marginTop='-10px' paddingRight="10px" > 
                                  <Typography style={{textAlign:'right', color:'#917EBD', fontSize:'24px'}}><CalendarMonthIcon style={{color: '#DFD1E7'}}/>{event.session_date.substring(0, 11)}</Typography>
                                  <Typography style={{float:'right', color:'#917EBD', fontSize:'24px'}}><ScheduleIcon style={{color: '#DFD1E7'}}/> {event.session_date.substring(12, 16)}</Typography>
                              </Grid>
                            </Grid>
                          </Box>
                        )
            ...
            <Box mt={'30px'} ml="20px" className = 'sub_div dark:text-white'>
              <Typography fontSize={"32px"} className='section_heading' fontWeight="bold">Course completion certificate</Typography>
              <p className='loggedhome_subheading'>Don’t have any certificates yet? Start a course now and earn your completion certificate </p>
            </Box>
        </Box>
    )};
};
