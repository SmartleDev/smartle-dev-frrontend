...
const UpdateParent = () => {
    ...
    const  handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        console.log(updateDetails.phone);
        console.log(updateDetails.code);
        // await API.post('updateparentinfo', {phone: updateDetails.phone,parent_id: parent?.username})
        //   .then((res) => {
    ...
    return (
        <Box >
            ...
            >
                    ...
                    <input
                        type={"text"} 
                        placeholder={parentInfo && parentInfo[0]?.parent_name}
                        style={ {background: "#F9EDF5", height: '40px',borderRadius: "3px", padding: "8px", cursor:"not-allowed" } }
                        name = 'name'
                        value={parentInfo && parentInfo[0]?.parent_name} 
                        disabled={true}
                    />
            ...
            <input
                type={"email"} 
                placeholder={parentInfo && parentInfo[0]?.parent_email}
                style={ {background: "#F9EDF5", height: '40px',borderRadius: "3px", padding: "8px", cursor:"not-allowed", border: '1px solid #917EBD' } }
                name = 'email'
                value={parentInfo && parentInfo[0]?.parent_email} 
                disabled={true}
            />

        <label htmlFor="code" style={{marginTop: '20px'}}>
          <Typography fontSize={20} fontWeight={800}>Country Code</Typography>
          <select
            id="code"
            style={ {background: "#F9EDF5", height: '40px',borderRadius: "3px", padding: "8px", border: '1px solid #917EBD' } }
            name = 'code'
            value={updateDetails.code} 
            onChange={handelChange} 
          >
            <option value="" hidden>Select Country Code</option> 
