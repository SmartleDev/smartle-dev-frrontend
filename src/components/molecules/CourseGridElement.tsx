...
const CourseGridElement = ({ course, width='sm:w-1/2 md:w-1/3 lg:w-3/12', color='#D8F2FF' }: Props) => {
    return (
        <div className={`${width} md:p-3`}>
            <Link className={``} to={`course/${course?.slug}`}>
                <div style={{height:'22rem', backgroundColor : '#D8F2FF'}}  className={`rounded-lg w-full shadow-xl relative md:p-4`}>
                    {
                        course?.credits ? (<>
                    ...
                    <div className="relative overflow-y-hidden h-full">
                        ...
                        {
                            course.image ? (<div style = {{position : 'absolute' , bottom:"0"}}>
                                <img className="w-full h-32 z-10 relative rounded-t-lg md:rounded-md" loading="lazy" src={course.image} alt={course.title} />
                                <div className='absolute top-0 w-full h-32 rounded-t-lg md:rounded-md'></div>
                            </div>) : (
                                <div className='w-full h-32 rounded-t-lg md:rounded-md'></div>
                            )
                        }
                    </div>
                </div>
